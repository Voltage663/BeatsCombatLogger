...
class Main extends PluginBase implements Listener{
    ...
    public function onEnable(){
        ...
        $this->getServer()->getLogger()->info("BeatsCombatLogger Enabled!");
        $this->getServer()->getPluginManager()->registerEvents($this, $this);
        $this->getServer()->getScheduler()->scheduleRepeatingTask(new Scheduler($this, $this->interval), 20);
    }

    public function onDisable(){
        $this->getServer()->getLogger()->info("BeatsCombatLogger Disabled!");
    }
    ...
    private function setTime(Player $player){
        $msg = "§l§c(!)§r §cLogging out now will cause you to die!\n§l§c(!)§r §cPlease wait §4".$this->interval." §cseconds...§r";
    ...
    public function PlayerCommandPreprocessEvent(PlayerCommandPreprocessEvent $event){
        if(isset($this->players[$event->getPlayer()->getName()])){
            ...
            if(isset($this->blockedcommands[$cmd])){
                $event->getPlayer()->sendMessage("§l§c(!)§r §cYou cannot use this command during combat!§r");
                $event->setCancelled();
            }
        }
    }
